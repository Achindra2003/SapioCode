...
/**
 * The actual running backend is CogniCode Intelligence Engine v2.4.0
...
function agentChat(body: {
  thread_id: string;
  mode: "hint" | "chat" | "viva_start" | "viva_answer";
  user_message: string;
  current_code: string;
...
export const aiApi = {
  chat: async (request: ChatRequest): Promise<ChatResponse> => {
    const res = await agentChat({
      ...
      frustration_score: Math.min(request.frustration_score, 1.0),
