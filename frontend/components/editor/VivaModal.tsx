...
export default function VivaModal({
  ...
  const startViva = async () => {
    ...
    try {
      const response = await vivaApi.start(threadId, code, 3);
      setQuestions(response.questions);
      setTotalQuestions(response.questions.length || 3);
      setCurrentQuestion(response.first_question);
      setQuestionNumber(1);
      setPhase("question");
    } catch (err: any) {
      console.error("Failed to start viva:", err);
      setErrorMsg(err.message || "Failed to start viva session");
      setPhase("error");
    }
  };
  ...
  const processAnswer = async (answerText: string) => {
    ...
    try {
      const result = await vivaApi.answer(threadId, answerText, code);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="w-full max-w-2xl bg-[#0d1117] border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
          <div>
            <h2 className="text-xl font-bold text-white">Viva Verification</h2>
        ...
        <div className="p-6 min-h-[300px]">
          ...
          {phase === "initializing" && (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              ...
              <span className="text-gray-400">Analyzing your code and generating questions...</span>
            </div>
          )}
              ...
              {!useTextMode ? (
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <button
                    ...
                    className="group w-20 h-20 rounded-full bg-emerald-600 hover:bg-emerald-500 flex items-center justify-center transition-all hover:scale-105 shadow-[0_0_30px_rgba(16,185,129,0.3)]"
                  >
                ...
                <div className="space-y-3">
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 resize-none"
                  />
                  <button
            ...
            <div className="space-y-4">
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span
                    className={`text-sm font-bold ${lastFeedback.is_acceptable ? "text-emerald-400" : "text-amber-400"}`}
                  >
                ...
                {lastFeedback.matched_concepts.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {lastFeedback.matched_concepts.map((c) => (
                      <span key={c} className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded">
                        {c}
                      </span>
                    ))}
                  </div>
            ...
            <div className="text-center py-6">
              {(finalVerdict === "pass" || finalVerdict === "PASS") && (
                <>
                  ...
                  <p className="text-gray-400 mb-4">You&apos;ve demonstrated clear understanding of your code.</p>
                </>
              )}
              {(finalVerdict === "weak" || finalVerdict === "NEEDS_DETAIL") && (
                <>
                  ...
                  <p className="text-gray-400 mb-4">You understood some concepts but missed a few key details.</p>
                </>
              )}
              {(finalVerdict === "fail" || finalVerdict === "FAIL") && (
                <>
                  ...
                  <p className="text-gray-400 mb-4">Review the concepts and try explaining your code again.</p>
                </>
              )}
                  ...
                  {answers.map((a, i) => (
                    <div key={i} className="bg-white/5 rounded-lg p-3 text-sm">
                      <div className="flex justify-between text-gray-400 mb-1">
                        <span>Q{i + 1}</span>
                        <span className={a.score >= 0.5 ? "text-emerald-400" : "text-red-400"}>
