    ...
    const rendered = parts.map((part, j) => {
      ...
      return italicParts.map((ip, k) => {
        if (ip.startsWith("*") && ip.endsWith("*") && ip.length > 2) {
          return (
            <em key={`${j}-${k}`} className="text-emerald-300/80">
              {ip.slice(1, -1)}
            </em>
          );
        }
  ...
  const contextPills = useMemo(() => {
    ...
    if (code.trim().length > 20) {
      pills.push({
        ...
        label: "Code loaded",
        color: "text-emerald-400/70 bg-emerald-500/10 border-emerald-500/10",
      });
    }
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      setChatHistory((prev) => [
        ...
        {
          role: "assistant",
          content: `I couldn't reach the AI service. ${err.message || "Please try again."}`,
        },
      ]);
    } finally {
      setIsLoading(false);
    }
  }, [input, isLoading, threadId, userId, code, compilerOutput, frustrationScore, problemDescription, failedTestCases, editorContext]);
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full w-[540px] z-50 flex flex-col
        bg-[#020408]/90 backdrop-blur-3xl border-l border-white/[0.08]
    ...
    >
      ...
      <div className="shrink-0 flex items-center justify-between px-8 py-5 border-b border-white/5 bg-white/[0.01]">
        <div className="flex flex-col">
          ...
          <span className="text-[10px] font-bold text-emerald-500/40 uppercase tracking-[0.3em] mt-0.5">
            Socratic Tutor
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onClose}
            className="w-10 h-10 rounded-lg bg-white/5 flex items-center justify-center hover:border-emerald-500/30 border border-white/10 transition-all"
          >
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
        ...
        {chatHistory.length === 0 && !isLoading && (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 border border-emerald-500/10 flex items-center justify-center mb-6">
        ...
        {chatHistory.map((m, i) => (
          ...
          >
            {m.role === "assistant" && (
              <div className="w-7 h-7 rounded-lg bg-emerald-500/10 flex items-center justify-center shrink-0 mt-1 mr-3">
                <Brain className="w-3.5 h-3.5 text-emerald-500/60" />
              </div>
            )}
            <div
              className={`max-w-[85%] rounded-2xl px-5 py-3.5 text-[14px] leading-relaxed ${
                m.role === "user"
                  ? "bg-emerald-600/90 text-white font-medium rounded-br-md"
        ...
        <div className="flex items-end gap-3">
          <textarea
            ...
            className="flex-1 bg-white/[0.04] border border-white/[0.06] rounded-xl px-5 py-3.5
              ...
              focus:outline-none focus:border-emerald-500/30 focus:bg-white/[0.06] transition-all"
            style={{ maxHeight: "160px" }}
          />
          <button
            onClick={send}
            disabled={!input.trim() || isLoading}
            className="h-12 w-12 shrink-0 rounded-xl bg-emerald-600 flex items-center justify-center
              hover:bg-emerald-500 active:scale-95 shadow-[0_8px_30px_rgba(5,150,105,0.3)]
